...
// URI of its own. It is written explicitly rather than omitted so the Go and
// Python services produce byte-comparable error bodies.
...
func problem(ctx context.Context, err error) error {
	...
	// Python service returns 504 here because asyncio cancels the task
	// outright; reporting the same status keeps the two agreeing on a case a
	// client actually retries on.
...
// for field, and the Python service emits the same object.
