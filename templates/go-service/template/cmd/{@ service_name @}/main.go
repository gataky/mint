// Command {@ service_name @} is the service entrypoint.
//
...
{% if include_examples %}	"{@ module_path @}/internal/repository/memory"
	"{@ module_path @}/internal/service"
{% endif %}	transport "{@ module_path @}/internal/transport/http"
)
...
{% if include_examples %}	// Repositories, then the services that use them, then the transport. This
...
{% else %}	// Repositories, then the services that use them, then the transport, are
	// named together here and nowhere else. Construct each resource's
	// repository and service in this block and pass them to NewAPI.

{% endif %}	health := transport.NewHealth()
	...
	apiMux := transport.NewAPI(cfg, {% if include_examples %}widgets, orders, {% endif %}logger)
