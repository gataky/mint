...
| `make build` | builds `bin/{@ service_name @}` |
...
```
cmd/{@ service_name @}/main.go            composition root — wires everything, owns signals

internal/domain/                  entities and the error taxonomy
{% if include_examples %}  widget.go  order.go             (imports nothing else in the service)
{% endif %}internal/service/                 business rules; declares the repository interfaces
{% if include_examples %}  widget.go  order.go
{% endif %}internal/repository/memory/       repository implementations
{% if include_examples %}  widget.go  order.go
{% endif %}internal/transport/http/          handlers, middleware, error mapping, listeners
{% if include_examples %}  widget.go  order.go
{% endif %}
...
default. A business-rule violation returns **400** — a bad *reference* in a
well-formed request to a route that does exist is not a missing resource. Every
response carries an `X-Request-Id` header, echoing the inbound one if there was
one.
...
`server.read_timeout` from `server.read.timeout`. A constant `{@ env_prefix @}_` prefix
