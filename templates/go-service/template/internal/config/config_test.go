...
func TestEnvironmentBeatsYAML(t *testing.T) {
	...
	t.Setenv("{@ env_prefix @}_LOGGING__LEVEL", "debug")
...
func TestDoubleUnderscoreSeparatesLevels(t *testing.T) {
	...
	// descends a level.
	t.Setenv("{@ env_prefix @}_SERVER__READ_TIMEOUT", "42s")
...
func TestUnprefixedEnvironmentVariablesAreIgnored(t *testing.T) {
	...
	t.Setenv("PORT", "1234")
	t.Setenv("{@ service_name|upper|replace('-', '_') @}_PORT", "tcp://10.0.162.149:8080")
...
func TestInvalidConfigurationIsAStartupError(t *testing.T) {
	t.Setenv("{@ env_prefix @}_ENV", "production") // the valid value is "prod"
...
func TestPrintAnnotatesTheWinningSource(t *testing.T) {
	...
	for _, want := range []string{
		...
		"env:{@ env_prefix @}_LOGGING__LEVEL", // ...and says so
...
func TestOTLPEndpointFallsBackToTheEcosystemVariable(t *testing.T) {
	// The service defers on OTLP *transport*: an operator who has already set the
...
func TestServiceVariableBeatsTheEcosystemVariable(t *testing.T) {
	t.Setenv("OTEL_EXPORTER_OTLP_ENDPOINT", "http://ecosystem:4318")
	t.Setenv("{@ env_prefix @}_OBSERVABILITY__TRACING__OTLP_ENDPOINT", "http://explicit:4318")
	...
	if got := loaded.Config.Observability.Tracing.OTLPEndpoint; got != "http://explicit:4318" {
		t.Errorf("otlp_endpoint = %q, want the {@ env_prefix @}_ variable to win", got)
	}
}

func TestServiceIdentityEnvironmentVariablesAreIgnored(t *testing.T) {
	// The service owns identity. Logs and spans disagreeing about service or env would
