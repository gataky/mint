...
Run it with ``python -m {@ package_name @}``, never the ``uvicorn`` CLI — the CLI
...
from {@ package_name @}.log import configure as configure_logging
from {@ package_name @}.log import logger
from {@ package_name @}.observability import Metrics, configure_tracing
{% if include_examples %}from {@ package_name @}.repository import memory
from {@ package_name @}.service import Orders, Widgets, random_id, system_clock
{% endif %}
...
def main(argv: list[str] | None = None) -> int:
    ...
    # nothing else.
{% if include_examples %}    widget_repo = memory.Widgets()
...
{% else %}    # thing_repo = memory.Things()
    # things = Things(thing_repo, random_id, system_clock)
{% endif %}
    health = Health()
    metrics = Metrics(config)

    try:
{% if include_examples %}        asyncio.run(serve(config, listeners(config, widgets, orders, health, metrics), health))
{% else %}        asyncio.run(serve(config, listeners(config, health, metrics), health))
{% endif %}    except Exception as exc:
...
def listeners(
    config: Config,
{% if include_examples %}    widgets: Widgets,
    orders: Orders,
{% endif %}    health: Health,
    metrics: Metrics,
) -> list[Listener]:
    ...
    connection-refused.
    """
{% if include_examples %}    api = create_api(config, widgets, orders, metrics)
{% else %}    api = create_api(config, metrics)
{% endif %}    admin = create_admin(config, health, metrics)
