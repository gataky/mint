...
``server.read_timeout`` from ``server.read.timeout``. The constant ``MINT_``
...
class Service(BaseSettings):
    ...
    name: str = Field(default="widget-svc", min_length=1)
    version: str = "0.1.0"
    owner: str = "unowned"
...
class Server(BaseSettings):
    ...
    admin_port: int = Field(default=9080, ge=1024, le=65535)
...
class Config(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        ...
        # An unknown key in YAML is a typo worth failing on. An unknown MINT_
